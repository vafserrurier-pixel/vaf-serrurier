...
export default function ServiceGrid({
  ...
  return (
    <div>
        ...
        {featured.map(({ href, label }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className="group block bg-navy/40 rounded-2xl border border-cream/15 p-6 sm:p-8 hover:border-cream/40 hover:bg-navy/60 transition-colors"
            >
              <span className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-cream/15 text-cream mb-4">
                <card.Icon className="w-6 h-6" />
              </span>
              <h3 className="font-heading font-bold text-cream text-lg mb-2">{label}</h3>
      ...
      {rest.length > 0 && (
        <div className="mt-6 pt-6 border-t border-cream/15 flex flex-wrap gap-2.5">
          {rest.map(({ href, label }) => (
            <Link
              key={href}
              href={href}
